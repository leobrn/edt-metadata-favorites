...
![EDT Metadata Favorites — избранное и Git-фильтр для 1С:EDT](docs/img/readme-banner.png)

Плагин для 1С:EDT, который добавляет избранное для проектов и объектов метаданных, а также
фильтр объектов с незакоммиченными изменениями Git.
...
- [добавление в избранное](docs/user-guide.md#как-добавлять-и-удалять-из-избранного) проектов,
  отдельных объектов метаданных, в том числе прямо из открытого редактора;
...
- режим [«Только изменённые Git»](docs/user-guide.md#фильтр-только-изменённые-git), который
  показывает объекты с незакоммиченными изменениями;
...
Избранное [хранится в рабочем пространстве EDT](docs/user-guide.md#где-хранится-избранное) и не
...
- **Несколько конфигураций в одном рабочем пространстве.**
...
- **Локальная задача в большой конфигурации.**
...
- **Проверка локальных изменений.**
  Включите Git-фильтр, чтобы увидеть изменённые объекты метаданных и перейти к ним
  в компактном дереве.

Основные действия описаны в компактном [руководстве пользователя](docs/user-guide.md).
...
`edt-metadata-favorites-<version>.zip` из выпуска на GitHub и добавьте его кнопкой `Archive`.
...
`.build/releases/<version>/` комплект для выпуска на GitHub:

- update-site ZIP для установки через 1С:EDT;
- bundle JAR для установки через `dropins`;
- `release-notes.md` из раздела `Unreleased` истории изменений;
- `SHA256SUMS.txt`.

Каталоги `target/` содержат только промежуточные результаты Maven. Версия релиза определяется из
