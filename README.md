...
**Параллельная доработка обмена между конфигурациями.** В одном окне формы управления избранным
закрепляете общие модули сразу в двух проектах - Навигатор показывает только их.

![Параллельная доработка обмена между двумя конфигурациями](docs/img/exchange-bp-unf.png)
...
- **Параллельная доработка обмена.** Закрепите связанные общие модули отдельно в каждой из
  конфигураций (например, в БП и УНФ), чтобы видеть обе стороны обмена в одном компактном дереве
  и быстро переходить между ними.
